...
const url = process.env.TURSO_DATABASE_URL;const isVercel = process.env.VERCEL === '1';
if (!url && isVercel) throw new Error('TURSO_DATABASE_URL is required on Vercel. Add TURSO_DATABASE_URL and TURSO_AUTH_TOKEN in Project Settings → Environment Variables.');
...
`INSERT OR IGNORE INTO site_sections(page,type,title,subtitle,body,sort_order,visible) VALUES ('home','hero','We turn ideas into powerful digital systems.','ShoTech Solutions • Digital Systems','We design and build modern websites, custom business systems and digital experiences that help ambitious businesses move faster.',10,1),('home','services','Technology designed around your business.','WHAT WE BUILD','From the first idea to a production-ready system, we build solutions with purpose.',20,1),('home','about','About ShoTech','WHO WE ARE','We build practical digital products and custom systems around the real needs of ambitious businesses.',30,1),('home','why','Why ShoTech','WHY CHOOSE US','Custom-built solutions, clear communication, scalable architecture and a process focused on measurable results.',40,1),('home','process','How we work','OUR PROCESS','Discover, Design, Build and Launch — a simple process with a strong result.',50,1),('home','projects','Systems built to perform.','FEATURED WORK','Explore selected projects built to solve real business problems.',60,1),('home','cta','Let’s build something useful','START A PROJECT','Tell us what you want to build and we will shape the right digital solution.',70,1),('about','about','Smart solutions. Powerful systems.','','ShoTech Solutions builds practical digital products that help businesses operate better, move faster and grow with confidence.',10,1),('process','process','A clear process. A stronger result.','','Discover, Design, Build and Launch — with every step managed around your business goals.',10,1)`
];
...
// Custom sections remain repeatable.
...
for(const [page,types] of coreByPage){
  for(const type of types){
    await db.execute(`DELETE FROM site_sections WHERE page=? AND type=? AND id NOT IN (SELECT MIN(id) FROM site_sections WHERE page=? AND type=?)`,[page,type,page,type]);
  }
}
// Enforce the same rule at database level so a repeated seed cannot recreate the problem.
try{await db.execute(`CREATE UNIQUE INDEX IF NOT EXISTS idx_site_sections_core_unique ON site_sections(page,type) WHERE type IN ('hero','services','about','why','process','projects','cta')`)}catch{}
