...
}, ['site-settings'], { revalidate:CACHE_REVALIDATE_SECONDS, tags:[siteTags.settings] });
...
const getSectionsCached = unstable_cache(async (page:string) => {
  const placeholders=CORE_SECTION_TYPES.map(()=>'?').join(', ');
  const sql=`SELECT * FROM site_sections WHERE page=? AND visible=1 AND (type NOT IN (${placeholders}) OR id IN (SELECT MIN(id) FROM site_sections WHERE page=? AND type IN (${placeholders}) GROUP BY type) OR ? <> 'home') ORDER BY sort_order,id`;
  return (await query(sql,[page,...CORE_SECTION_TYPES,page,...CORE_SECTION_TYPES,page])).rows as any[];
}, ['site-sections'], { revalidate:CACHE_REVALIDATE_SECONDS, tags:[siteTags.sections('home'),siteTags.sections('about'),siteTags.sections('why'),siteTags.sections('process')] });
