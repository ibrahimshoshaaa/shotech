  ...
  return <div className="admin-shell" dir="rtl">
    <aside className="sidebar">
      ...
      <nav className="admin-nav" aria-label="التنقل داخل لوحة التحكم">
        {nav.map(([href,label,Icon])=><Link key={href} href={href} className={pathname===href||pathname.startsWith(href+'/')?'active':''}><Icon size={19}/><span>{label}</span></Link>)}
      </nav>
      <div className="admin-bottom"><Link href="/" target="_blank"><ExternalLink size={16}/> فتح الموقع</Link></div>
    </aside>
    <section className="admin-content">
      <header className="admin-topbar"><div className="admin-top-title"><Menu size={20}/><strong>لوحة تحكم ShoTech</strong></div><Link href="/" target="_blank" className="admin-top-link">عرض الموقع <ExternalLink size={15}/></Link></header>
