...
CREATE TABLE IF NOT EXISTS projects (
  id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT NOT NULL, title_en TEXT DEFAULT '', slug TEXT NOT NULL UNIQUE,
  excerpt TEXT, excerpt_en TEXT DEFAULT '', content TEXT, content_en TEXT DEFAULT '', cover_image TEXT, images TEXT DEFAULT '[]',
  technologies TEXT DEFAULT '[]', category TEXT, category_en TEXT DEFAULT '', demo_url TEXT, github_url TEXT,
  status TEXT DEFAULT 'draft', featured INTEGER DEFAULT 0, sort_order INTEGER NOT NULL DEFAULT 0,
  created_at TEXT DEFAULT CURRENT_TIMESTAMP, updated_at TEXT DEFAULT CURRENT_TIMESTAMP
);
CREATE TABLE IF NOT EXISTS services (
  id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT NOT NULL, title_en TEXT DEFAULT '', description TEXT, description_en TEXT DEFAULT '',
  content TEXT DEFAULT '', content_en TEXT DEFAULT '', slug TEXT, cover_image TEXT DEFAULT '', icon TEXT DEFAULT 'Code2',
  sort_order INTEGER DEFAULT 0, visible INTEGER DEFAULT 1, created_at TEXT DEFAULT CURRENT_TIMESTAMP
);
CREATE TABLE IF NOT EXISTS messages (
  id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, email TEXT NOT NULL, phone TEXT, company TEXT DEFAULT '',
  subject TEXT DEFAULT '', message TEXT NOT NULL, status TEXT DEFAULT 'unread', created_at TEXT DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE IF NOT EXISTS site_sections (
  ...
  sort_order INTEGER DEFAULT 0, visible INTEGER DEFAULT 1, created_at TEXT DEFAULT CURRENT_TIMESTAMP, updated_at TEXT DEFAULT CURRENT_TIMESTAMP
);
CREATE INDEX IF NOT EXISTS idx_messages_created_at ON messages(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_projects_created_at ON projects(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_projects_status_featured ON projects(status, featured);
CREATE INDEX IF NOT EXISTS idx_projects_sort_visible ON projects(sort_order, status, featured);
...
('companyName','ShoTech Solutions'),('companyName_en','ShoTech Solutions'),('tagline','حلول ذكية. أنظمة قوية.'),('tagline_en','Smart solutions. Powerful systems.'),
('email',''),('whatsapp',''),('phone',''),('address',''),('address_en',''),('working_hours',''),('working_hours_en',''),
...
('seoDescription','حلول رقمية ذكية وأنظمة قوية تساعد عملك على النمو.'),('seoDescription_en','Smart digital solutions and powerful systems that help businesses grow.'),
...
('processBody','نفهم، نصمم، نبني ونطلق حلولًا رقمية حول احتياجات كل مشروع.'),('processBody_en','We discover, design, build and launch digital solutions around the real needs of each business.');
